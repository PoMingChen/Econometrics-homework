...
```{r}
#訂正完畢，用group_by。
#union他是指工會的這個變數，不是指union取聯集的function
...
labordata %>% group_by(UNION) %>% summarise(average_LWAGE = mean(LWAGE), average_WKS = mean(WKS))
```
...
```{r, results='asis'}
#訂正完畢，記得chunk設定要打個,/提醒要調整成robust standard error的解釋
library(sandwich)
library(lmtest)
library(stargazer)
...
#需要調整成robust standard error，也就是存在異質變異下的也一樣可以使用。因為實際研究中不相信現實生活中所謂的齊值變異（各變數的母體標準差相同），是一定有異質變異的，自然需要調整。
...
```
＃訂正完畢：文字敘述中，效應的描述都要先講「在其他變數不變下，一個given的概念」
在複迴歸模型中，係數是0.538，個別檢定是不顯著的。

LWAGE的詮釋是「在其他變數不變下，每當工資變動（增加）1%，每週工時會增加多少小時」，在複回歸底下是0.538小時，約32分鐘。
```
...
均衡薪資和均衡每週工時，是勞動市場均衡的條件。其中薪資同時受到供給和需求影響，因此就算在勞動供給線控制再多控制變數仍然不會得到一致性估計，因為不滿足計量的「非薪資效應之每週工時」與「薪資效應」的條件獨立。所以需要選擇一個只會影響勞動需求，而不會影響勞動供給的工具變數。我會選擇「廠商使用高科技技術程度」作為工具變數。
```
...
```
訂正完畢：讓他變得更solid一點。然後要留意，這邊「產業別」變數是講製造業，所以文字敘述也要以製造業作為陳述。同理，居住地(smsa)的變數也是。

工具變數需要留意他的相關條件與排除條件。

「產業別」與「薪資」具有相關性，因為產業的榮枯前景不同，自然會影響從業人員的薪水。同時，他也與「非薪資效應之每週工時」無關，理由是因為一個勞工不會因為他身處「製造業」就因此特別愛做事或者不愛做事。

「居住地」與「薪資」具有相關性，因為待在大城市機會比較多，產業群聚也可能較佳，自然薪水是水漲船高。同時，他也與「非薪資效應之每週工時」無關，理由是因為勞動者應該不會因為「住在都市（或稱都市的勞動者）」就因此特別愛工作，或者特別不愛工作。
...
```{r one IV合理性檢定}
#原則上iverg，已經是有異質變異調整過後的robust standard error，所以可以不用再調整standard error
